...
```{r, results='markup', echo=TRUE, fig.align='center'}
...
```{r, results='markup', echo=TRUE, fig.show='hold', fig.width=8, fig.height=3, fig.align='center'}
par(mfrow=c(1, 3))
...
```{r, results='markup', echo=TRUE, fig.show='hold', fig.width=5, fig.height=3, fig.align='center'}
...
From both of the histograms of prices by cab company, we can have an overall sight of the relationship between prices and cab company in terms of Uber and Lyft.

For cab company Lyft, there is a distribution that is right-skewed. It seems that most of the trip prices of Lyft are distributed between 5 to 30 dollars. The frequency peaks above 600 when the prices fall in 10 to 15 dollars. 

For cab company Uber, there is a distribution that is more right-skewed. It seems that most of the trip prices of Uber are distributed between 5 to 20 dollars. The frequency peaks above 900 when the prices fall in 5 to 10 dollars. 

In comparison, the prices for Uber are more concentrated than the prices for Lyft. The average prices of Uber rides are also lower than the average prices of Lyft rides. 

Therefore, there is an obvious difference between the riding price of two ride-sharing companies. In the later data EDA, we will have a deeper look at the relationship between cab company and prices to see whether they are also affacted by the cab types.
...
```{r, results='markup', echo=TRUE, fig.width=5, fig.height=5, fig.align='center'}
...
And Cab_types and prices in Uber:
...
```{r, results='markup', echo=TRUE, fig.width=5, fig.height=3, fig.align='center'}
...
When conducting data analysis and statistical research, measuring the standard deviations of the data and whether the distribution of the data satisfies the normal distribution Test normality is usually a key link. Both factors play an important role in determining the appropriateness of statistical analyses, selecting appropriate statistical methods, and interpreting study results. In the following sections, we explore the role of standard deviation and normality tests in data analysis.

First, we will use standard deviation to evaluate the dispersion of price data to understand how volatile prices are under different conditions. The analysis of standard deviation will help us determine the stability and reliability of the data and provide a basis for further statistical analysis.

Second, the normality test is used to determine whether the data conform to the assumption of normal distribution. Normal distribution is of great significance in statistical analysis. By performing a normality test, we can understand whether the data distribution is close to a normal distribution, or whether there is an obvious deviation, to ensure that we choose appropriate statistical methods and correctly interpret the research results.

Through the analysis of standard deviations and normality tests, we will better understand the nature of the data and the appropriateness of statistical analysis. This will help ensure the reliability of our research methods and results.
...
We will next use Correlation Heatmap to explore the relationship between the four key numerical variables of price, surge_multiplier, distance and time hours. These variables play an important role in the field of shared mobility services, and the relationship between them is crucial for understanding price fluctuations and pricing strategies.
By using a heat map, we will visually demonstrate the correlation between these four variables. We will examine the following question: Is there a correlation between price and surge_multiplier? Is ride distance significantly related to price? Are hours related to price or other factors? Are these relationships positive or negative? 

This research will reveal the relationships between data through heat maps, providing a basis for further data analysis and decision-making.

```{r, results='markup', echo=TRUE, fig.width=5, fig.height=4, fig.align='center'}
...
From the correlation heatmap for numeric data, looking at the rightest column, distance variable has the highest correlation with price which is 0.35. Surge_multiplier variable has the second highest correlation with price which is 0.26. However, for hour variable, it has the lowest correlation with price which is -0.04.

Therefore, the hour variable has the lowest relationship with price. In the later research, linear regression analysis, we are not going to choose hour as a deciding variable in terms of predicting the price.

## Chi-Squared Test

Chi-Squared Test is a powerful tool for studying relationships between categorical data. In this study, we will use the chi-square test to explore the relationship between Cab_company and Cab_type.

Cab_company and Cab_type are two important classification variables in the field of shared travel services. Taxi companies may offer different services, and the type of online car hailing usually reflects the comfort of the vehicle, pricing strategy, and ride experience. Using the chi-square test, we will explore the following question: Is there an association between Cab_companies and Cab_types? Is the relationship between these two categorical variables significant? The chi-square test will help us determine the degree of correlation between Cab_companies and Cab_types and prices, and whether they influence each other.
...
Chi-Squared Value: Chi-Squared (X-squared) value is 6000. This value represents a statistic calculated by the chi-square test that measures the deviation between observed and expected data. Large Chi-Squared values usually indicate significant differences between observed and expected values.

Degrees of Freedom (df) is 11. In the Chi-Squared test, the degrees of freedom represent the amount of independent information used to calculate the chi-square value. In this case, there are 11 degrees of freedom, which are related to the relationship between different companies and car types.

p-Value: p-Value is less than 2e-16, which is close to zero. p-Value measures whether the difference between observed and expected data is significant. Here, a p-Value close to zero indicates that the observed differences are significant, i.e., the relationship between Cab_company and Cab_type is unlikely to arise by chance.

Based on the results of this Chi-Squared Test, we can draw the following conclusions:
There is a significant correlation between Cab_company and Cab_type. A p-Value close to zero indicates that the relationship between the two categorical variables is unlikely to occur by chance but is caused by some systematic association.

A Chi-Squared value of 6000 indicates that the observed and expected data deviate significantly. This means that the distribution varies significantly between cap_companies and cap_types.

Taken together, the results of this Chi-Squared Test indicate that the relationship between Cab_companies and Cab_types is significant and deserves further research and analysis.
...
# Reference Page
1. Brllrb. (2019). Uber and Lyft dataset: Boston, MA [Data set]. Kaggle. https://www.kaggle.com/datasets/brllrb/uber-and-lyft-dataset-boston-ma/data
