...
plot(df_hour_price$hour, df_hour_price$mean_price, type='l',
     xlab='hour', ylab='mean price', main='hour VS mean price')
```
...
# Adjust the angle and size of x-axis labels
p + theme_minimal() +
  theme(legend.position = "none") +
  theme(plot.title = element_text(hjust = 0.5, size = 14)) +
  theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 0.5)) +  # Adjust the angle
...
plot_ly(data = df, x = ~hour, y = ~distance, z = ~price, type = "scatter3d", mode = "markers", marker = list(size = 3)) %>%
